...
centers_table <- here("inst/shiny_interface/data/2016_italy.RDS") %>%
  read_rds() %>%
  mutate(
    across(p_accept, ~if_else(.x != 0, acceptance_rate, .x)),
    mr = if_else(region == "Lombardia", TRUE, FALSE),
    mr_name = if_else(region == "Lombardia", "Lombardia", ""),
    regpos = if_else(region == "Veneto", 5L, regpos),
    macropos = if_else(region == "Veneto", NA_integer_, macropos),
    inmacropos = if_else(region == "Veneto", "", inmacropos),
    inmacropos = if_else(region == "Lombardia", "1", inmacropos),
  )
...
ggplot2::ggsave(here("analyses", "output",
                     paste0("2016_noNIT_p-by-n_", acceptance_rate, "accept.png")),
...
ggplot2::ggsave(here("analyses", "output",
                     paste0("2016_noNIT_p_at-least-n_", acceptance_rate, "accept.png")),
                width = 11.7, height = 8.3
)
